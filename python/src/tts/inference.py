import sys
sys.path.append('/usr/src/app/tts/')
...
def infer(weights, vocoder, denoiser, text, speaker,
          ...
          plot, seed, file_name, data_config_p, model_config_p):
    # os.makedirs(output_dir, exist_ok=True)

    torch.manual_seed(seed)
    # weights = torch.load(radtts_path, map_location='cpu')

    radtts = RADTTS(**model_config_p)
    ...
    trainset = Data(
        data_config_p['training_files'],
        **dict((k, v) for k, v in data_config_p.items() if k not in ['training_files', 'validation_files'])
    )
    ...
    return process_folder(output_dir, vocoder, denoiser, denoising_strength, file_name, mel)
